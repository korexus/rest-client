...
class RestClient {
  constructor(baseURL, endpoints) {
    ...
    callable.forEach(endpoint => {
      self[endpoint] = async ({ user, args, context, auth }) => (
        self.call({ user, endpoint, args, context, auth })
      );
    });
  }
  ...
  async call({ user, endpoint, args, context, auth }) {
    let authOptions = {};
    if (auth) {
      authOptions = { ...auth }
    } else if (user) {
      const _auth = await user.getAuth();
      authOptions = { ..._auth };
    }
    const fullContext = { user, ...args, ...context };
    return this._call(endpoint, args, authOptions, fullContext);
  }  
}
