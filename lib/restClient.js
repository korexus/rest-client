...
class RestClient {
  ...
  _endpointDetails(endpoint) {
    const [path, method, transformFunctions, methodFunctions] = this._endpoints[endpoint];
    const transforms = (transformFunctions || []).map(t => this[t]);
    const handlers = methodFunctions || {};
  ...
  async _processResponse(response, transforms=[], handlers={}, context={}) {
    ...
    if (handlers[response.status]) {
      return handlers[response.status](response, context);
    }
